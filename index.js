...
function attachResilience(connection) {
    ...
    connection.on('stateChange', async (oldState, newState) => {
        if (newState.status === VoiceConnectionStatus.Disconnected) {
            try {
                // Discord.js voice can end up "disconnected" either because
                // it's about to move/resume (recoverable) or because it was
                // actually kicked (not recoverable). Try to recover first.
                await Promise.race([
                    entersState(connection, VoiceConnectionStatus.Signalling, 5000),
                    entersState(connection, VoiceConnectionStatus.Connecting, 5000),
                ]);
                // Still recovering, do nothing — it'll reconnect on its own.
            } catch {
                // Recovery didn't happen in time — clean it up so !track /
                // !rwa7 / !jc can be used again without a stuck connection.
                try {
                    connection.destroy();
                } catch (err) {
                    console.error('❌ Failed to destroy stuck connection:', err.message);
                }
